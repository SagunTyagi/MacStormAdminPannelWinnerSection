...
const initialProblems = [
  {
    ...
    date: "2025-07-22",
    assigned: "Unassigned",
    description: "User unable to process payout to bank account. Shows error code 504.",
  },
  {
    id: "P002",
    ...
    description: "User claims incorrect result in match ID 2201, team performance mismatch.",
  },
  {
    id: "P003",
...
const adminUsers = ["admin_swati", "admin_ravi", "admin_ajay", "admin_sneha", "admin_rohan"];
...
const ProblemCenter = () => {
  ...
  const filteredProblems = problems.filter((p) =>
    ...
    (p.title.toLowerCase().includes(search.toLowerCase()) || p.user.toLowerCase().includes(search.toLowerCase()))
  );
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="overflow-x-auto rounded shadow mt-4">
        <table className="min-w-full bg-white text-sm">
          <thead className="bg-gray-100 text-gray-600">
            <tr>
              {["ID", "Title", "User", "Category", "Severity", "Status", "SLA", "Date", "Assigned", "Action"].map((h) => (
            ...
            {paginated.map((p) => (
              <tr key={p.id} className="border-t hover:bg-gray-50">
                ...
                <td className="px-4 py-2">
                  <span className={`px-2 py-1 text-xs rounded-full text-white ${p.severity === "High" ? "bg-red-500" :
                ...
                <td className="px-4 py-2">{p.sla} ({p.timer})</td>
      ...
      {modalProblem && (
        <Modal problem={{ ...modalProblem, createdAt: modalProblem.date }} onClose={() => setModalProblem(null)} />
      )}
    </div>
  );
};
